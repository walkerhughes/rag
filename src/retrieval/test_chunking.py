...
from retrieval.chunking import LONGEST_CHUNK_WORDS, TARGET_WORDS, Turn, chunk_turns


def turn(position: int, words: int, speaker: str = "Dwarkesh Patel") -> Turn:
    return Turn(position=position, speaker=speaker, text=" ".join(["word"] * words))


def test_short_turns_are_packed_together() -> None:
    """A quarter of real turns are under twenty words, which retrieve badly alone."""
    chunks = chunk_turns([turn(i, 10) for i in range(20)])
...
def test_a_turn_longer_than_the_ceiling_is_split() -> None:
    ...
    segment = Turn(position=0, speaker="Richard Sutton", text=sentences)

    chunks = chunk_turns([segment])
...
def test_every_chunk_records_the_turns_it_covers() -> None:
    chunks = chunk_turns([turn(i, 60) for i in range(10)])
...
def test_chunks_are_numbered_from_zero_in_order() -> None:
    chunks = chunk_turns([turn(i, 80) for i in range(12)])
...
def test_speakers_are_recorded_in_order_without_repeats() -> None:
    ...
    chunk = chunk_turns(segments)[0]
    assert chunk.speakers == ["Dwarkesh Patel", "Richard Sutton"]


def test_the_first_turns_timestamp_carries_onto_the_chunk() -> None:
    segments = [
        Turn(position=0, speaker="A", text="Hello.", start=timedelta(minutes=5)),
        Turn(position=1, speaker="A", text="More.", start=timedelta(minutes=6)),
    ]
    assert chunk_turns(segments)[0].start == timedelta(minutes=5)
...
def test_chunking_is_deterministic() -> None:
    ...
    assert chunk_turns(segments) == chunk_turns(segments)
